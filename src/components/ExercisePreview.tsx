...
function noteHeadY(syllable: Stroke["syllable"]): number {
  if (syllable === "DUM") return 520; // grave: lower note = longer stem
  if (syllable === "TA") return 485;  // agudo: higher note = shorter stem
  return 505;                          // relleno: x-note in the middle
}
...
function drawLegend(): string {
  ...
  const legendText = (label: string, y: number) => text(legendTextX, y, label, 40, "end");
  const legendLabel = (label: string, y: number) => text(legendLabelX, y, label, 38, "start", 500);
...
export function renderExerciseSVGString(exercise: Exercise, activeStep?: { barIndex: number; strokeIndex: number } | null): string {
  ...
  let svg = `
    ...
    ${text(95, 470, topNum, 86, "middle")}
    ${text(95, 555, bottomNum, 86, "middle")}
